...
class news():
    ...
    def nytimes_tail(self,tail_href, label_name):
        ...
        if not res:
            ...
            head_image = head_data_json['image'][0]['url']
            res = re.match('.*/(.*?\.\w+).*?', head_image).group(1)
            image_image = head_image.split('/')[-5] + '-' + head_image.split('/')[-4] + '-' + head_image.split('/')[
                -3] + '-' + head_image.split('/')[-2] + '-' + res
            root = ".\\image\\"
            ...
            if not os.path.exists(path):
                with open(path, 'wb') as f:
                    while True:
                        try:
                            if requests.get(url=head_image, verify = False, proxies=proxies, timeout=30
                                            ).content:
                                f.write(requests.get(url=head_image, verify = False, proxies=proxies,
                                                     timeout=30
                                                     ).content)
                                break
                            else:
            ...
            nytime_json['header_image_src'] = image_url
            contents = []
            if myhtml.xpath(
                    ...
                    if content_section[c_s].xpath('./div/p/text()|./div/p/a/text()|./text()|./a/text()|./p/text()'):
                        ...
                        if content_section[c_s].xpath('./div/p/a/text()'):
                            ...
                            if c_s_content:
                                contents.append(c_s_content)
                        continue
                    elif content_section[c_s].xpath('./div/figure/div/picture/img/@src|./div/figure/@itemID|./div/figure/@itemid|./figure/@itemid|./div/img/@src'):
                        c_s_images = content_section[c_s].xpath('./div/figure/div/picture/img/@src|./div/figure/@itemID|./div/figure/@itemid|./figure/@itemid|./div/img/@src')

                        if c_s_images:
                            for c_s_image in c_s_images:
                            #c_s_image = c_s_image[0]
                                res = re.match('.*/(.*?\.\w+).*?', c_s_image).group(1)
                                image_image = c_s_image.split('/')[-5] + '-' + c_s_image.split('/')[-4] + '-' + \
                                            c_s_image.split('/')[-3] + '-' + c_s_image.split('/')[-2] + '-' + res
                                root = ".\\image\\"
                                path = root + image_image
                                if not os.path.exists(root):
                                    os.mkdir(root)
                                if not os.path.exists(path):
                                    with open(path, 'wb') as f:
                                        while True:
                                            try:
                                                if requests.get(url=c_s_image, verify = False, proxies=proxies, timeout=30
                                                                ).content:
                                                    f.write(requests.get(url=c_s_image, verify = False, proxies=proxies,
                                                                        timeout=30
                                                                        ).content)
                                                    break
                                                else:
